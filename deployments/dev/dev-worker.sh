...
CONFIG_FILE="$SCRIPT_DIR/worker.config.yaml"
CONFIG_TEMPLATE="$SCRIPT_DIR/worker.config.example.yaml"
if [ ! -f "$CONFIG_FILE" ]; then
    echo -e "${YELLOW}Config file not found. Creating from template...${NC}"
    cp "$CONFIG_TEMPLATE" "$CONFIG_FILE"
    echo -e "${GREEN}worker.config.yaml created from worker.config.example.yaml${NC}"
...
cd "$ROOT_DIR"
LEROS_DEV=true ./bundles/leros worker --worker-id 1 --config "$CONFIG_FILE" --server-addr 127.0.0.1:8080 --listen-addr :8081
