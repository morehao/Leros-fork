...
// SessionMetadataProviderSessionStore 在 Session.Metadata.Extra 中存储提供者会话绑定。
type SessionMetadataProviderSessionStore struct {
	db *gorm.DB
}

// NewSessionMetadataProviderSessionStore 创建一个由 Session.Metadata 支持的提供者会话存储。
...
// Get 返回存储在 Leros 会话元数据上的提供者会话绑定。
...
// Upsert 在 Session.Metadata.Extra 上存储或替换提供者会话绑定。
...
// MarkFailed 保持提供者绑定不变，因为失败状态不会被持久化。
