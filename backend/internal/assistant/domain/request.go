...
// Slices and maps are copied so the clone does not share mutable backing data with the original.
func CloneRequest(req *RunRequest) *RunRequest {
	...
	// SystemPrompt and EventSink are not deep-copied (string is immutable, EventSink is an interface).
	return &clone
}
